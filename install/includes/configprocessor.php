...
$configDir = dirname(realpath(__DIR__.'/../../includes/common.php'));
echo "Checking folder permissions (". $configDir .")...";
flush();
clearstatcache();
...
if (!is_writable(realpath(__DIR__.'/../../logs'))) {
	$validerror .= "<br><strong>Cannot write to the logs directory. Please check the /logs/ folder permissions. See the RPints Installation page on www.raspberrypints.com.</strong>";
}
if (!is_writable(realpath(__DIR__.'/../../templates_c'))) {
	$validerror .= "<br><strong>Cannot write to the template cache directory. Please check the /templates_c/ folder permissions. See the RPints Installation page on www.raspberrypints.com.</strong>";
}
...
if ($action == 'install') {
	...
	file_put_contents(__DIR__.'/../../includes/dbconfig.php', $mainconfigstring);
	...
	*/
	...
	if(!empty($_POST['sampledata'])) {
		...
		$dbms_schema = __DIR__."/../../sql/test_data.sql";
		$sql_query = @fread(@fopen($dbms_schema, 'r'), @filesize($dbms_schema)) or die('Cannot find SQL schema file. ');
